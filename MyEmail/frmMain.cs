...
namespace MyEmail
{
    public partial class frmMain : Form
    {
        ...
        private void search_Click(object sender, EventArgs e)//通过关键字来搜索相关邮件
        {
           ...
           else
            {
                for (i = 0; i < row; i++)
                {
                    dgvEmailInfo.Rows[i].Selected = false;//清空选中属性
                }
            }
        }
        public static bool bl;
        private void frmMain_FormClosing(object sender, FormClosingEventArgs e)//点击关闭窗体触发函数
        {           
            if (bl == false)
            {
        ...
        private void draftToolStripMenuItem1_Click(object sender, EventArgs e)//点击进入草稿箱
        {
            draft dt = new draft();
            dt.Show();
        }
    
        private void sentToolStripMenuItem_Click(object sender, EventArgs e)//点击进入已发送
        {
            sent st = new sent();
            st.Show();
        }

        private void bookToolStripMenuItem_Click(object sender, EventArgs e)//点击查看联系人
        {

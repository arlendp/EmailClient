...
namespace MyEmail
{
    public partial class login : Form
    {
        ...
        private string GetResponse()//接收服务器的响应
        {
            string str = null;
            try
            {
        ...
        private void btnCancel_Click(object sender, EventArgs e)//点击取消退出程序
        {
            Application.Exit();
        }

        private void Username_SelectedIndexChanged(object sender, EventArgs e)//选中用户名时触发的函数
        {
            OleDbDataAdapter da2 = new OleDbDataAdapter("select * from users where 用户名='" + Username.Text + "'", sqlCon);
            DataSet ds2 = new DataSet();
            da2.Fill(ds2, "tablename1");
            txtPwd.Text = ds2.Tables["tablename1"].Rows[0]["密码"].ToString();//填充密码
            if (txtPwd.Text != "")
            {
            ...
            {
                checkBox1.Checked = false;//配置checkbox属性
            }
        }               
    }
}
